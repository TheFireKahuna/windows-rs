...
> Direct2D drawing: the device pair, drawing sessions, geometry and geometry realizations, brushes, bitmap targets and sprite batches.
...
built on. Render targets are FP16 with premultiplied alpha, so colour stays in linear scRGB and
values above paper white survive to the compositor.
