...
//!
//! Run with:
//!   cargo run -p windows-reactor --example dcomp_caption --features dcomp-backend
...
fn main() -> windows_reactor::Result<()> {
    ...
    DCompHost::render("NewAPO — DComp caption", app)
}

#[cfg(not(feature = "dcomp-backend"))]
fn main() {
    eprintln!(
        "dcomp_caption requires the `dcomp-backend` feature:\n  \
         cargo run -p windows-reactor --example dcomp_caption --features dcomp-backend"
    );
}
