...
use windows_composition::{ContainerVisual, Visual};
...
pub(crate) enum PopupBody {
    ...
    Text(windows_canvas::TextLayout),
...
pub(crate) fn layout_text_body(s: &str) -> Option<windows_canvas::TextLayout> {
    ...
    let layout = windows_canvas::TextLayout::new(s, &fmt, column, 100_000.0).ok()?;
    Some(layout)
}
...
impl Popup {
    ...
    fn layout(
        ...
        let (w, h) = match body {
            PopupBody::Menu(items) => {
                ...
                (anchor.width().clamp(200.0, 360.0), h)
            }
    ...
    fn build_surface(
        ...
        container.set_offset(panel.left - MARGIN, panel.top - MARGIN, 0.0);
        container.set_size(surf_w, surf_h);
        Ok((container, surf))
    }
    ...
    ///
    /// Infallible now: the overlay container IS a visual — it derefs to one —
    /// so the cast that used to be able to fail (and whose failure branch tore
    /// the overlay down un-faded) has no counterpart to fail.
    pub fn into_exit(self, comp: &Compositing) -> Visual {
        let cfg = AnimationConfig {
            opacity: Some(0.0),
            from_opacity: None,
            scale: None,
        ...
        animate::start(comp.compositor(), &self.container, &cfg, None);
    ...
    /// [`Self::content_inset`].
    pub fn adopt(&self, content: &Visual) {
        self.container.children().insert_at_top(content);
    }
    ...
    /// The removal is fallible-and-ignored, as it always was: the subtree may
    /// already have been detached (its node destroyed, or the overlay container
    /// replaced under it by a re-layout), and for this call "already gone" is
    /// the goal state rather than a failure.
    pub fn release_content(&self, content: &Visual) {
        let _ = self.container.children().try_remove(content);
    }
    ...
    fn paint_surface(&self, comp: &Compositing) {
        let (ctx, (origin_x, origin_y)) =
            match self.surf.surface.begin_draw::<ID2D1DeviceContext>() {
                Ok(c) => c,
                Err(e) => {
                    comp.note_error(&e);
                    return;
        ...
        if let Err(e) = self.surf.surface.end_draw() {
            comp.note_error(&e);
        }
    }
