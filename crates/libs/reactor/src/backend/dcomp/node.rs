...
use windows_canvas::{ColorF, TextLayout};
...
pub(crate) fn linear(c: Color) -> ColorF {
    let [r, g, b, a] = color_out::apply([c.r, c.g, c.b, c.a]);
    ColorF::new(r, g, b, a)
}
...
pub(crate) struct Node {
    ...
    ///
    /// Offset/size/opacity go straight through it: a `ContainerVisual` derefs to
    /// the `Visual` that owns those properties, so the cached `IVisual` view this
    /// used to carry beside it has no counterpart and is gone.
    ...
    pub button_text: Option<Box<glyph_text::ButtonText>>,
    ...
    pub text_part: Option<Box<glyph_text::TextPart>>,
    ...
    pub live_run: Option<Box<glyph_text::Shaped>>,
    ...
    pub item_text: Option<Box<glyph_text::ItemText>>,
    /// Select triggers only: every label a `ComboBox`/`DropDownButton` could
    /// show, its chevron, and the two parts that place them.
    ///
    /// Separate from `item_text` because a select shows ONE of its labels at a
    /// time rather than all of them — one part that re-places, not a part per
    /// item — and separate from `text_layout` because the run the measure pass
    /// wants (the widest candidate) is not the run the draw wants (the selected
    /// one). See [`glyph_text::SelectText`](super::glyph_text::SelectText).
    pub select_text: Option<Box<glyph_text::SelectText>>,
    ...
    pub knob: Option<Box<knob::KnobParts>>,
    ...
    pub path: Option<Box<path_shape::PathParts>>,
    ...
    pub ring: Option<Box<ring_shape::RingParts>>,
    ...
    ///
    /// Unlike every other kind's, these are NOT built by the layout pass — all
    /// four sizes derive from the dial's radius, which is not known until the
    /// solve has run. See [`glyph_text::KnobText`](super::glyph_text::KnobText).
    pub knob_text: Option<Box<glyph_text::KnobText>>,
    /// Editors only: the placeholder shown while the field is empty.
    ///
    /// Its own run rather than a second state of `text_part`, because the two
    /// are different text with different ink and the editor's part also carries
    /// the selection wash and the composition rule — state a placeholder has no
    /// business inheriting. A [`Shaped`](super::glyph_text::Shaped) because its
    /// only inputs are the words and the em, and it is placed by the pen rather
    /// than by DirectWrite alignment, so the content width it used to be laid
    /// out against is no longer one of them.
    pub placeholder: Option<Box<glyph_text::Shaped>>,
    ...
    pub scroll_thumb: Option<Box<parts::Part>>,
    ...
    pub scroll_spring: Option<SpringVector3NaturalMotionAnimation>,
    ...
    pub spring_anim: Option<SpringVector3NaturalMotionAnimation>,
    ...
    pub caption_glyphs: Option<Box<glyph_text::CaptionGlyphs>>,
    ...
    pub bar_glyphs: Option<Box<glyph_text::BarText>>,
}
...
impl Node {
    ...
    /// node's own into this node's own collection, recording it in
    /// [`Self::stacked`] as one operation.
    ///
    /// A birth site that mints an owned visual — chrome parts, the scroll thumb
    /// — parents it immediately, because the next re-stack may never come: a
    /// mint dirties nothing, so a part inserted at the wrong end would read that
    /// way indefinitely. But a visual belongs to one collection at a time, so a
    /// visual parented behind the registry's back is one that the next re-stack
    /// neither detaches (it is not in `stacked`) nor can insert (it is still a
    /// child) — the collision that `restack`'s infallible insert would panic on.
    ///
    /// Registering here is what forecloses that: whatever these methods parent,
    /// the next re-stack detaches. Insert through them for anything `collect`
    /// names, and NOT for a stray the sync pass must leave alone (a Knob's arc
    /// and needle, an editor's caret) — a stray's whole contract is that
    /// `stacked` has never heard of it.
    pub fn adopt_at_top(&mut self, v: &Visual) {
        self.container.children().insert_at_top(v);
        self.stacked.push((layout::Slot::Container, v.clone()));
    }

    /// [`Self::adopt_at_top`], onto the floor of the collection.
    pub fn adopt_at_bottom(&mut self, v: &Visual) {
        self.container.children().insert_at_bottom(v);
        self.stacked.push((layout::Slot::Container, v.clone()));
    }

    /// [`Self::adopt_at_top`], directly above a `sibling` already in the
    /// collection.
    pub fn adopt_above(&mut self, v: &Visual, sibling: &Visual) {
        self.container.children().insert_above(v, sibling);
        self.stacked.push((layout::Slot::Container, v.clone()));
    }
    ...
    /// Everything a knob's text sync reads, borrowed together — the same split
    /// [`ctrl_and_parts`](Self::ctrl_and_parts) exists for, over three fields
    /// instead of two.
    ///
    /// A knob shapes its runs at placement time from strings it does not own a
    /// copy of: the unit, the sub-line and every tick label live in `ctrl`, and
    /// the readout is `paint.text`. Handing them back beside the mutable text is
    /// what lets the sync compare against them in place — cloning the readout to
    /// dodge the borrow would put an allocation on the value-drag path, which is
    /// the one path here that runs every frame.
    pub fn knob_runs(&mut self) -> (&Ctrl, &str, &mut glyph_text::KnobText) {
        (
            self.ctrl.as_deref().unwrap_or(EMPTY_CTRL),
            &self.paint.text,
            self.knob_text.get_or_insert_default(),
        )
    }

    /// The placeholder's words and the run that shows them, borrowed together —
    /// the same split [`ctrl_and_parts`](Self::ctrl_and_parts) exists for.
    pub fn placeholder_run(&mut self) -> (&str, &mut glyph_text::Shaped) {
        (
            self.ctrl.as_deref().unwrap_or(EMPTY_CTRL).placeholder.as_str(),
            self.placeholder.get_or_insert_default(),
        )
    }
    ...
    pub fn push_offset(&mut self, x: f32, y: f32) {
        ...
        {
            // Disjoint fields: the container is read while the cached spring
            // beside it is written. The cache is threaded in rather than rebuilt
            // so a continuous relayout allocates nothing per pass.
        ...
        self.container.set_offset(x, y, 0.0);
        self.last_off = Some((x, y));
        if first {
    ...
    pub fn scroll_snap(&mut self, offset: f32) {
        ...
        if let Some(c) = &self.scroll_content {
            c.stop_animation("Offset");
            c.set_offset(0.0, -offset, 0.0);
        }
    ...
    pub fn attach_implicit_if_ready(&mut self) {
        ...
        if let Some(coll) = &self.implicit {
            self.container.set_implicit_animations(Some(coll));
    ...
    pub fn set_implicit(&mut self, coll: Option<ImplicitAnimationCollection>) {
        ...
        if self.implicit.is_none() {
            if self.implicit_attached {
                self.container.set_implicit_animations(None);
            }
