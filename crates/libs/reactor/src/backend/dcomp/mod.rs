...
use windows_composition::{CompositionScopedBatch, Visual};
...
/// source alive); for a dismissed popup it is the overlay container
...
impl DCompBackend {
    ...
    pub(crate) fn set_root(&mut self, root: Option<ControlId>) {
        if root != self.attached_root {
            ...
            {
                self.comp.attach_root(&n.container);
    ...
    pub(crate) fn back_button_rect(&self) -> Option<(f32, f32, f32, f32)> {
        ...
        let r = caption::back_rect(
            n.extras(),
            windows_canvas::Rect::from_xywh(n.rect.x, n.rect.y, n.rect.w, n.rect.h),
        )?;
    ...
    fn spawn_exit_ghost(&mut self, id: ControlId) {
        ...
        let mut cur = (*node.container).clone();
        ...
        // `remove_child`) no longer knows this visual either way. The removal is
        // best-effort: a parent already torn down leaves nothing to detach.
        let source = (*container).clone();
        if let Some(parent) = container.parent() {
            let _ = parent.children().try_remove(&source);
        }

        // A FLATTENED ghost: one visual-surface sprite re-compositing the
        // detached subtree, so the exit fade is a single layer and overlapping
        // translucent children cannot bleed through mid-fade. The sprite's
        // brush chain keeps the source subtree alive; only the base `Visual` is
        // retained here, which is all the fade and the release need.
        let sprite = animate::snapshot_sprite(self.comp.compositor(), &source, w, h);
        let shown: Visual = (**sprite).clone();

        // Present top-level at the same on-screen position.
        self.comp.attach_root_visual(&shown);
        shown.set_offset(ax, ay, 0.0);
    ...
    fn park_ghost(&mut self, shown: Visual, batch: CompositionScopedBatch) {
        ...
        if let Some(revoker) = revoker { self.ghosts.push(Ghost { id, shown, _completed: revoker }) } else {
...
impl DCompBackend {
    ...
    fn build_node(&mut self, kind: ControlKind) -> Node {
        let container = self.comp.new_container();
        ...
        if node.is_scroll() {
            node.scroll_content = Some(self.comp.new_container());
        }
        node
    }
...
impl Backend for DCompBackend {
    ...
    fn run_property_animation(&mut self, id: ControlId, config: Option<AnimationConfig>) {
        ...
        animate::start(self.comp.compositor(), &node.container, &cfg, center);
    }
...
pub(crate) fn apply_prop(node: &mut Node, prop: Prop, value: &PropValue) -> bool {
    ...
    match (prop, value) {
        ...
        (Prop::Ticks, PropValue::F64List(list)) => {
            node.ctrl_mut().ticks.clone_from(list);
            node.mark_dirty();
        }
        (Prop::TickLabels, PropValue::ValueLabels(list)) => {
            node.ctrl_mut().tick_labels.clone_from(list);
            node.mark_dirty();
        }
        ...
        (Prop::Unit, PropValue::Str(s)) => {
            node.ctrl_mut().unit.clone_from(s);
            node.mark_dirty();
        }
        (Prop::SubText, PropValue::Str(s)) => {
            node.ctrl_mut().sub_text.clone_from(s);
            node.mark_dirty();
        }
        ...
        (Prop::Message, PropValue::Str(s)) => {
            node.extras_mut().message.clone_from(s);
            node.text_dirty = true;
            node.mark_dirty();
        }
        ...
        (Prop::PlaceholderText, PropValue::Str(s)) => {
            node.ctrl_mut().placeholder.clone_from(s);
            // A ComboBox SHAPES its placeholder — it is the label an unselected
            // trigger shows, and it is one of the candidates the trigger is
            // sized to. An editor's placeholder is still painted, and rebuilding
            // a run it does not use costs one shaped string on a prop that
            // changes about never.
            node.text_dirty = true;
            node.mark_dirty();
        }
        (Prop::Items, PropValue::StrList(list)) => {
            node.ctrl_mut().items.clone_from(list);
            // A ComboBox's items ARE its labels: every one is shaped, the widest
            // is what it measures to, and the selected one is what its sprites
            // place. Without this the trigger kept the runs it was born with and
            // a list that arrived after birth was never shaped at all.
            node.text_dirty = true;
            node.mark_dirty();
            // A focused AutoSuggestBox whose filtered list just changed refreshes
        ...
        (Prop::MenuItems, PropValue::NavMenuItems(items)) => {
            ...
            for it in items {
                ...
                ctrl.icons.push(it.icon.map_or(0, |s| s.0 as u32));
            }
            sync_selected_tag(node);
            node.mark_dirty();
        }
        ...
        (Prop::Title, PropValue::Str(s)) => {
            node.extras_mut().title.clone_from(s);
            node.text_dirty = true;
            node.mark_dirty();
        }
        (Prop::Subtitle, PropValue::Str(s)) => {
            node.extras_mut().subtitle.clone_from(s);
            node.text_dirty = true;
            node.mark_dirty();
        }
        ...
        (Prop::PaneTitle, PropValue::Str(s)) => {
            node.extras_mut().pane_title.clone_from(s);
        ...
        (Prop::AutoSuggestItems, PropValue::StrList(list)) => {
            node.extras_mut().suggest_items.clone_from(list);
            node.mark_dirty();
        }
        (Prop::AutoSuggestPlaceholder, PropValue::Str(s)) => {
            node.extras_mut().suggest_placeholder.clone_from(s);
            node.mark_dirty();
        }
        ...
        (Prop::NavigateUri, PropValue::Str(s)) => {
            node.extras_mut().navigate_uri.clone_from(s);
        }
        (Prop::OnContent, PropValue::Str(s)) => {
            node.extras_mut().on_content.clone_from(s);
            node.text_dirty = true;
            node.mark_dirty();
        }
        (Prop::OffContent, PropValue::Str(s)) => {
            node.extras_mut().off_content.clone_from(s);
            node.text_dirty = true;
            node.mark_dirty();
        }
...
prop_contract! {
    ...
    consumed {
        ...
        // Shaped by a ComboBox (it is the unselected trigger's label), so losing
        // it has to re-run the text pass.
        PlaceholderText => |n| {
            n.ctrl_reset(|c| c.placeholder = Ctrl::DEFAULT.placeholder);
            n.text_dirty = true;
        }
...
fn menu_row(def: &crate::MenuItemDef) -> MenuRow {
    ...
    match def {
        ...
        M::Item {
            ...
            text: text.clone(),
            tag: text.clone(),
            icon: icon.map_or(0, |s| s.0 as u32),
