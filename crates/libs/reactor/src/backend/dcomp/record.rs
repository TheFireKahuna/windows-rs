...
///
/// Two `PropValue` variants hold thread-affine payloads — `SurfaceImageSource`
/// wraps a COM interface, `FlyoutDef` holds a `Box<Element>` and a callback — so
/// they cannot ride the buffer. Both are
...
pub(crate) enum SendValue {
    ...
    CanvasOffset(crate::CanvasOffset),
...
impl SendValue {
    ...
    fn from_prop(value: &PropValue) -> Option<Self> {
        Some(match value {
            ...
            PropValue::CanvasOffset(v) => Self::CanvasOffset(*v),
            ...
            // Thread-affine COM interface, with no consumer on this backend.
            PropValue::SurfaceImageSource(_) => return None,
        })
    }
    ...
    fn into_prop(self) -> PropValue {
        match self {
            ...
            Self::CanvasOffset(v) => PropValue::CanvasOffset(v),
...
impl Backend for RecordingBackend {
    ...
    fn set_prop(&mut self, id: ControlId, prop: Prop, value: &PropValue) {
        ...
        // A `None` is dropped on purpose: thread-affine prop values (XAML image
        // sources, flyout Element trees) have no consumer on this backend —
        // `DCompBackend::set_prop` has no arm for any of them — so there is
        // nothing to declare and nothing to keep. If the backend ever grows one,
        // `from_prop`'s exhaustive match is where the decision resurfaces.
        if let Some(value) = SendValue::from_prop(value) {
            self.push(Cmd::SetProp { id, prop, value });
        }
    }
    ...
    fn set_pointer_handlers(&mut self, id: ControlId, handlers: Option<&PointerHandlers>) {
        let interest = if let Some(h) = handlers {
