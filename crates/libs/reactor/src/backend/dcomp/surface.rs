...
//!
//! The compositor and the host container handed to the factory are the safe
//! [`windows_composition`] wrappers the backend already holds (the node arena's
//! `ContainerVisual`, the bootstrap's `Compositor`); nothing here reaches for the
//! raw generated bindings.
...
impl SurfaceHost {
    ...
    #[allow(
        ...
        reason = "rebuild-in-place and append are symmetric cases, each with its \
                  own comment"
    )]
    fn factory_for(
        &mut self,
