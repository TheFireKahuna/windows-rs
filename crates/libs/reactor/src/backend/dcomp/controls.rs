...
fn paint_button(session: &DrawingSession, brush: &Brush, node: &Node, rect: Rect, dim: f32) {
    ...
    let base = if accent {
        ...
        theme::accent_fill()
    } else if matches!(node.kind, ControlKind::Button | ControlKind::RepeatButton) {
        // Subtle button: transparent at rest, wash appears via ink.
        theme::w(0.0)
    } else {
    ...
    let fg = node
        .paint
        .foreground
        .unwrap_or(if accent || checked { theme::ACCENT } else { theme::TEXT });
    text(
        session,
        brush,
        &node.paint.text,
