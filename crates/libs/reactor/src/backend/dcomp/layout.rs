...
#[allow(
    ...
    reason = "one layout pass's distinct inputs; bundling them would only move \
              the list behind a struct"
)]
fn compute_in(
...
pub(crate) fn rebuild_text(arena: &mut Arena, id: ControlId) {
    ...
    let needs_ornaments = arena
        .get(id)
        .is_some_and(|n| n.text_dirty && node::is_button_family(n.kind));
    if needs_ornaments {
        let icon = arena
            .get(id)
            .map(|n| n.extras().icon)
    ...
    if needs {
        let (text, size, weight, family, wrap) = {
            ...
            let (size, weight) = if node::is_button_family(n.kind) {
                (n.paint.font_size, controls::button_palette(n).weight)
    ...
    // A wide NumberBox's two spin chevrons, shaped into the positional `[up,
    // down]` shape the Expander's pair uses — and into `item_text`, which an
    // editor has no other use for. Neither the hover brighten nor a width change
    // that reveals the column sets `text_dirty`, so both have to exist up front.
    let needs_spin = arena
        .get(id)
        .is_some_and(|n| n.text_dirty && n.kind == ControlKind::NumberBox);
    if needs_spin {
        let built: Vec<Option<TextLayout>> = [controls::SPIN_GLYPH_UP, controls::SPIN_GLYPH_DOWN]
            .into_iter()
            .map(|cp| {
                controls::glyph_str(cp).and_then(|g| {
                    build_text_layout(&g, controls::SPIN_CHEVRON_SIZE, 400, theme::FONT_ICON, false)
                })
            })
            .collect();
        if let Some(n) = arena.get_mut(id) {
    ...
    if needs_select {
        ...
        // EVERY candidate is shaped, not merely the widest: the widest answers
        // the measure and the selected one is what the sprites place, and those
        // are the same run only by coincidence. Picking an item sets `dirty` but
        // not `text_dirty`, so a pass that kept only one of them would leave the
        // sync placing whichever run this happened to keep.
        //
        // Positional, and the placeholder is last — which is what
        // `SelectText::label_slot` indexes with `selected_index`.
        //
        // Shaped at the trigger's own size and weight, not the node's text
        // style, because that is what lands; measuring at a size the draw does
        // not use is how a label comes to overflow the box that was sized for it.
        let labels: Vec<Option<TextLayout>> = candidates
            .iter()
            .map(|s| {
                build_text_layout(
                    s,
                    controls::SELECT_FONT_SIZE,
                    controls::SELECT_WEIGHT,
                    &family,
                    false,
                )
            })
            .collect();
        // The chevron is shaped here too, for the reason the Expander's is: a
        // sprite is placed from a shaped run, and the painted trigger had only a
        // codepoint. Without one the chevron column renders nothing.
        let chevron = controls::glyph_str(controls::SELECT_CHEVRON).and_then(|g| {
            build_text_layout(
                &g,
                controls::SELECT_CHEVRON_SIZE,
                400,
                theme::FONT_ICON,
                false,
            )
        ...
        if let Some(n) = arena.get_mut(id) {
            let t = n.select_text.get_or_insert_with(Default::default);
            t.labels = labels;
            t.chevron = chevron;
            n.measure_dirty = true;
        }
    ...
    if needs_badge {
        ...
        let (size, weight) = arena
            .get(id)
            .map_or((info_badge::FONT_SIZE, info_badge::FONT_WEIGHT), |n| (n.paint.font_size, n.paint.font_weight));
...
impl LayoutTree {
    ...
    fn viewport(&mut self, width: f32, height: f32, root: NodeId, hug: bool) -> Option<NodeId> {
        let vp = if let Some(v) = self.viewport { v } else {
...
fn collect(arena: &Arena, id: ControlId, out: &mut Stack) {
    ...
    // `(**c).clone()` rather than `c.clone()`: a `ContainerVisual` derefs to the
    // `Visual` the stack is keyed by, but `Deref` loses to the inherent `Clone`
    // in method resolution, so the deref is spelled out. Same below for a
    // child's container.
    let carrier = n.scroll_content.as_ref().map(|c| (**c).clone());
    ...
    for (i, c) in n.children.iter().enumerate() {
        if let Some(cn) = arena.get(*c) {
            ...
            out.push((key, (*cn.container).clone()));
        }
    }
    if let Some(parts) = n.parts.as_deref() {
    ...
    if let Some(v) = n.scroll_thumb.as_ref().map(|s| s.visual()) {
...
fn restack(arena: &mut Arena, id: ControlId, order: &Stack) {
    let Some(n) = arena.get(id) else { return };
    // Both collections are infallible now — a container always has children.
    let coll = n.container.children();
    let carrier = n.scroll_content.as_ref().map(|c| c.children());
    ...
    let mut prev = arena
        ...
        .unwrap_or_default();
    // `prev` can name a visual that is no longer in the collection — a child
    // node destroyed since the last stack takes its container out from under us
    // — and for this loop "already detached" is the goal state, not a failure.
    // Hence `try_remove` rather than the panicking `remove`.
    for (slot, v) in prev.drain(..) {
        let _ = pick(slot).try_remove(&v);
    }
    ...
    // Infallible: a visual belongs to one collection at a time, so every visual
    // `collect` names must be unparented by the time it is laid down — which is
    // exactly what the detach above guarantees, because a birth site that
    // parents an owned visual registers it through [`Node::adopt_at_top`] and
    // friends rather than inserting behind the registry's back.
    for (k, v) in order.iter().rev() {
        pick(k.slot).insert_at_bottom(v);
    }
}
