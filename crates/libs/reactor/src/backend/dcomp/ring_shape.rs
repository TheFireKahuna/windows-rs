...
impl RingParts {
    ...
    fn start_revolve(&self, cx: f32, cy: f32) {
        let sprite = self.arc.display();
        sprite.set_center_point(Vector3::new(cx, cy, 0.0));
        let c = sprite.compositor();
        let lin = c.create_linear_easing_function();
        let a = c.create_scalar_key_frame_animation();
        a.insert_key_frame_with_easing(0.0, 0.0, &lin);
        a.insert_key_frame_with_easing(1.0, std::f32::consts::TAU, &lin);
        a.set_duration(super::parts::progress_cycle());
        a.set_iterate_forever();
        sprite.stop_animation("RotationAngle");
        sprite.start_animation("RotationAngle", &a);
    }
