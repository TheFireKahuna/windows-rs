...
//!   springs** for Offset / Size / Opacity. A state change is `set_final_value`
//!   plus `start_animation` on the cached object (no per-event allocation); a
//!   drag snap is `stop_animation` + a plain property set. Spring tuning matches the
...
/// Seconds as a [`Duration`](std::time::Duration), floored at 1 ms.
///
/// The floor was the old `TimeSpan` helper's, and it stays for the same reason:
/// a zero-length spring period or keyframe duration is not a fast animation, it
/// is an undefined one. The non-finite guard is new and load-bearing —
/// `Duration::from_secs_f32` PANICS on a NaN or negative input where the retired
/// `(s * 1.0e7) as i64` cast saturated silently, so the clamp that used to be a
/// nicety is now the difference between a floored duration and a crash.
/// (`f32::max` already returns the finite operand against a NaN; the explicit
/// test is what catches an infinity.)
...
fn rasterize(comp: &Compositing, key: &AtlasKey) -> Option<AtlasEntry> {
    ...
    // The surface is its own draw seam now — `begin_draw` / `end_draw` hang off
    // it, and the atlas origin comes back as a plain tuple rather than a `POINT`
    // filled in through an `unsafe` out-parameter.
    let (surface, brush) = comp.new_source_surface(px_w, px_h).ok()?;
...
mod channel {
    ...
    impl Channel {
        ...
        /// Record a plain property write of `t`.
        ///
        /// # Why this takes no `Result`
        ///
        /// This used to be handed the `HRESULT` of the `SetOffset` / `SetSize`
        /// it recorded, and advanced to `Verified` only on success. The rule was
        /// right; the evidence was the wrong evidence, and the safe composition
        /// wrapper removing that `Result` is what made the difference visible.
        ///
        /// A composition property set does not talk to the graphics device. It
        /// writes a value into the compositor's own copy of a visual, and the
        /// device-backed half — rasterizing pixels — is the Direct2D
        /// `begin_draw` / `end_draw` bracket, which is STILL fallible and is
        /// still checked (see [`rasterize`](super::rasterize)). So the one
        /// transient this backend genuinely sees, **device loss**, never
        /// surfaced from a property setter in the first place: composition
        /// objects outlive it, and it arrives through
        /// `Compositing::note_error`.
        ///
        /// What remains as a failure mode for a live visual's setter — the wrong
        /// apartment, a closed object, out of memory — is permanent rather than
        /// transient. Re-issuing the identical write on the next sync, which is
        /// exactly what the old rule bought, repairs none of them. (The wrapper
        /// panics on them, so the state this guarded against — a cache advanced
        /// past a write that never landed — is not reachable at all.)
        ///
        /// # The invariant is unchanged, and still enforced
        ///
        /// "A wedged part is impossible" holds on the mechanism that always did
        /// the real work: `DCompBackend` consumes `Compositing::took_device_loss`
        /// and calls `Parts::invalidate`, which
        /// [`reclaims`](Self::reclaimed) every channel. A reclaimed channel is
        /// `Nothing` — not evidence, and not `placed` — so the next sync writes
        /// unconditionally AND snaps. That is the whole repair path, and it is
        /// keyed on the fallible call that can actually report the damage rather
        /// than on one that cannot.
        pub(super) fn wrote(&mut self, t: (f32, f32)) {
            self.known = Known::Verified(t);
        }
    ...
    mod tests {
        ...
        /// The other half of that invariant, and the half that moved.
        ///
        /// Evidence used to be retired by a property setter reporting failure.
        /// It is retired by DEVICE LOSS now — the one event that can actually
        /// invalidate what this cache claims — which reaches a channel as
        /// [`Channel::reclaimed`] via `Parts::invalidate`. See
        /// [`Channel::wrote`] for why the setter's own result was never the
        /// right evidence.
        ///
        /// The failure this guards is the same one it always was: a channel
        /// still asserting `A` after the compositor it was talking to went away
        /// self-gates every later write of `A`, and the sprite is stranded for
        /// the life of the window.
        #[test]
        fn device_loss_retires_evidence_from_every_state() {
            // Verified.
            let mut c = Channel::new();
            c.wrote(A);
            c.reclaimed();
            assert!(!c.verified_at(A), "a landed write does not survive device loss");
            assert!(!c.placed(), "so the repairing write snaps rather than glides");

            // Mid-flight — the claim must not outlive the compositor either.
            let mut c = Channel::new();
            c.wrote(A);
            let f = c.arming();
            c.animating("Offset", B, f);
            c.reclaimed();
            assert!(!c.claims_flight_to(B), "a flight claim does not survive device loss");
            assert!(c.needs_retarget(B, false), "and the destination is written afresh");

            // Ceded to an out-of-band animation that is likewise gone.
            let mut c = Channel::new();
            c.wrote(A);
            c.ceded("Offset.X");
            c.reclaimed();
            assert!(!c.verified_at(A));
            assert_eq!(c.begin_snap(), None, "nothing is left holding the property");
        }
...
pub(crate) struct Part {
    sprite: SpriteVisual,
    /// The sprite as a plain `Visual` — the face every property write, animation
    /// start/stop and liveness query goes through.
    ///
    /// One field where there used to be two (`IVisual` + `ICompositionObject`):
    /// the safe wrapper's `Visual` carries the property setters AND the
    /// `start_animation` / `stop_animation` / `is_animating` / `compositor`
    /// operations that the raw path needed a second interface for. It is held
...
impl Part {
    pub(crate) fn new(comp: &Compositing) -> Self {
        let sprite = comp.new_sprite();
        // `SpriteVisual` derefs `SpriteVisual → ContainerVisual → Visual`, so
        // this is the same COM object the two `cast`s used to fetch, without the
        // two `QueryInterface` calls or the two failure paths that never fired.
        let vis = (**sprite).clone();
        Self {
            sprite,
            vis,
            nine: None,
    ...
    /// guessed: `Visual::is_animating` reports false once nothing animates the
    ...
    fn animation_live(&self, prop: &str) -> bool {
        self.vis.is_animating(prop)
    }

    fn open_batch(&self) -> CompositionScopedBatch {
        self.vis.compositor().create_scoped_batch(BatchKind::Animation)
    }
    ...
    ///
    /// Both halves stay fallible, and both still have to succeed: a batch
    /// subscribed to but never sealed swallows every later animation, and one
    /// sealed with no subscriber never reports. `None` means no completion will
    /// arrive, so the flight claim it was armed for can never be promoted to
    /// evidence — which is safe rather than wedging, because
    /// `Channel::needs_retarget` retires a claim the moment the compositor is
    /// observed to have no animation on the property.
    fn close_batch(
        batch: CompositionScopedBatch,
        settled: std::rc::Rc<std::cell::Cell<u32>>,
        flight: u32,
    ) -> Option<windows_core::EventRevoker> {
        let revoker = batch.on_completed(move || settled.set(flight)).ok()?;
        batch.try_end().ok()?;
        Some(revoker)
    }
    ...
    pub(crate) fn bind(&mut self, comp: &Compositing, atlas: &mut Atlas, key: AtlasKey) {
        ...
        if key.uses_nine_grid() {
            ...
            nine.set_insets(l, t, l, t);
            nine.set_inset_scale(1.0 / scale);
            // The sealed `Brush` trait is what the widening `cast` used to buy,
            // and it cannot fail: a surface brush IS a brush by type.
            nine.set_source(&entry.brush);
            self.sprite.set_brush(&nine);
        } else {
            self.sprite.set_brush(&entry.brush);
        }
        // Recorded unconditionally: the writes above cannot report failure, and
        // the only event that can invalidate the binding — a device loss — also
        // clears the atlas and bumps its epoch, so the gate at the top of this
        // method re-binds on the next sync. See `Channel::wrote`.
        self.key = Some(key);
        self.epoch = epoch;
    }
    ...
    pub(crate) fn glide(&mut self, x: f32, y: f32, w: f32, h: f32) {
        ...
        if self.off.needs_retarget((x, y), live) {
            ...
            self.glide_offset(x, y, dist);
            // No `place` fallback any more: starting a spring on a live visual
            // has no failure to fall back FROM. The batch may still fail to
            // report, and the flight claim then simply never becomes evidence —
            // `needs_retarget`'s liveness observation retires it instead.
            self._settle[0] = Self::close_batch(batch, cell, flight);
        ...
        if self.size.needs_retarget((w, h), live) {
            ...
            let batch = self.open_batch();
            self.glide_size(w, h, dist);
            self._settle[1] = Self::close_batch(batch, cell, flight);
            self.size.animating("Size", (w, h), flight);
        }
    }

    /// Retarget the cached Offset spring. The spring object is built ONCE and
    /// only retuned per move — minting a fresh one here would both allocate per
    /// event and reset the velocity, turning a redirection into a restart.
    fn glide_offset(&mut self, x: f32, y: f32, dist: f32) {
        if self.s_off.is_none() {
            let a = self.vis.compositor().create_spring_vector3_animation();
            a.set_damping_ratio(CHROME_SPRING_DAMPING);
            self.s_off = Some(a);
        }
        // Re-tuned per retarget, not once at construction: the period is a
        // function of THIS move's distance.
        let Some(a) = self.s_off.as_ref() else { return };
        a.set_period(secs(spring_period(dist)));
        a.set_final_value(Vector3::new(x, y, 0.0));
        self.vis.start_animation("Offset", a);
    }

    /// Retarget the cached Size spring — [`glide_offset`](Self::glide_offset)'s
    /// `Vector2` twin, same caching discipline.
    fn glide_size(&mut self, w: f32, h: f32, dist: f32) {
        if self.s_size.is_none() {
            let a = self.vis.compositor().create_spring_vector2_animation();
            a.set_damping_ratio(CHROME_SPRING_DAMPING);
            self.s_size = Some(a);
        }
        let Some(a) = self.s_size.as_ref() else { return };
        a.set_period(secs(spring_period(dist)));
    ...
    pub(crate) fn set_opacity(&mut self, a: f32) {
        ...
        if self.op_gliding {
            self.vis.stop_animation("Opacity");
            self.op_gliding = false;
        }
        // Cached unconditionally, on `Channel::wrote`'s reasoning — this is the
        // one property that is not a `Channel`, and `Part::invalidate` clears it
    ...
    fn loop_x(&mut self, from: f32, to: f32, cycle: f32) {
        ...
        // Recorded BEFORE the animation is started, exactly as it was: the
        // property is conceded by intent, not by the call succeeding.
        self.off.ceded("Offset.X");
        let comp = self.vis.compositor();
        let lin = comp.create_linear_easing_function();
        let a = comp.create_scalar_key_frame_animation();
        a.insert_key_frame_with_easing(0.0, from, &lin);
        a.insert_key_frame_with_easing(1.0, to, &lin);
        a.set_duration(secs(cycle));
        a.set_iterate_forever();
        self.vis.stop_animation("Offset.X");
        self.vis.start_animation("Offset.X", &a);
    }
...
impl Parts {
    ...
    pub(crate) fn below_visuals(&self) -> impl Iterator<Item = Visual> + '_ {
        self.below.iter().map(Part::visual)
    }
    pub(crate) fn above_visuals(&self) -> impl Iterator<Item = Visual> + '_ {
        self.above.iter().map(Part::visual)
    }
}
...
mod slot {
    /// The lower of the box's two sprites, and the upper one composited over it.
    ///
    /// Which of fill and border each holds depends on how the box is built —
    /// see [`super::box_slots`]. A stroked box draws its fill low and its
    /// outline over it; a ring-by-clip box draws the border low and the fill
    /// over it.
...
pub(crate) fn button_plan(node: &Node, scale: f32) -> PartPlan {
    ...
    let plate = super::controls::button_boxes(node, Rect::from_xywh(0.0, 0.0, w, h))
        .badge
        .zip(super::controls::badge_paint(node, &pal)).map_or_else(SlotPlan::hidden, |(b, (fill, _))| {
            let bh = b.height();
            AtlasKey::hbar(bh, bh / 2.0, 0.0, fill, scale).snap_at(Some(r4(b)), dim)
        });

    PartPlan::new([w, h, 0.0])
        .below(slot::LOWER, slots.lower)
        .below(slot::UPPER, slots.upper)
...
/// two agreed.
/// The two sprites a filled, bordered rounded box needs, plus where anything
/// stacked over the fill belongs.
pub(crate) struct BoxSlots {
    pub(crate) lower: SlotPlan,
    pub(crate) upper: SlotPlan,
    /// The rect and corner radius that a wash layered OVER the fill must use.
    ///
    /// It is the INNER box when the border was built as a ring of two clipped
    /// solids, and the whole box otherwise. A wash that always took the outer
    /// rect would paint over the ring in the first case and hide it.
    pub(crate) inner: (Rect4, f32),
    /// Whether the border was built as a ring, which is also what decides where
    /// a third layer goes: `[border, fill, wash]` when it was, `[fill, wash,
    /// border]` when it was not — the wash sits over the fill either way, but
    /// only a stroked border needs to stay on top of it.
    pub(crate) ringed: bool,
}

/// Plan a filled, bordered rounded box: a ring of two clipped solids where that
/// is exact, and a fill under a stroked raster where it is not.
///
...
/// `CompositionVisualSurface` capture per bordered box. It does not need one: a
/// border is the part of an outer rounded rect that an inner one does not
/// cover, so painting the border colour over the outer rect and the fill colour
/// over the inner leaves exactly the ring. Both are clipped solids, so the pair
/// costs no raster, no capture and no geometry — the same two sprites the
/// callers already bind.
///
/// The rects are CONCENTRIC, which is what makes the ring uniform: the outer
/// corner arc is centred `radius` in from the corner and the inner one
/// `t + (radius - t)`, the same point, so the ring is `t` wide through the
/// corners as well as along the sides. (Both radii are snapped independently by
/// [`AtlasKey::bar`], so the width can differ from `t` by under one detail step;
/// that grid is sub-pixel.)
///
/// The substitution is exact only where the fill hides the border beneath it,
/// which takes two facts a cache key cannot report and a plan can:
///
/// - an OPAQUE fill. A translucent one composites over the border rect instead
///   of the backdrop and tints the whole interior with the border colour; a
///   border with no fill at all degenerates to a solid disc.
/// - an UNDIMMED box. `dim` lands on each sprite separately, so at `dim < 1` the
///   interior composites the fill over a dimmed border over the backdrop rather
///   than over the backdrop alone.
///
/// Anything failing either test keeps the stroked raster, which is also the only
/// route for a ring with nothing opaque inside it — a focus ring, a toggle's
/// off-state track, an unchecked checkbox.
pub(crate) fn box_slots(
    rect: Rect4,
    radius: f32,
    ...
    // A fully transparent source binds nothing: an atlas source that paints no
    // pixels is a wasted raster and a wasted cache slot.
    ...
    let border = border.filter(|c| c.a > 0.0 && t > 0.0);
    let (x, y, w, h) = rect;

    if let (Some(f), Some(s)) = (fill, border)
        && f.a >= 1.0
        && dim >= 1.0
    {
        // The border is inset INTO the box, so the thing beneath it is the
        // fill — always, by construction. That is the ground the wash was
        // waiting for, so resolve it here instead of deferring: `over` gives
        // the colour the stroked raster would have composited, and an opaque
        // colour is one the ring can paint directly over the backdrop.
        //
        // Without this the ring is only exact for an already-opaque border,
        // which in a Fluent-derived palette is none of them — every stroke
        // token is ink at a low alpha. Resolving here is what makes the
        // construction apply to the whole control library rather than nothing.
        // It is idempotent on a border that is already opaque.
        let s = theme::over(s, f);
        let inner = (x + t, y + t, (w - 2.0 * t).max(0.0), (h - 2.0 * t).max(0.0));
        let inner_r = (radius - t).max(0.0);
        return BoxSlots {
            lower: SlotPlan::snap(AtlasKey::hbar(h, radius, 0.0, s, scale), Some(rect), dim),
            upper: SlotPlan::snap(
                AtlasKey::hbar(inner.3, inner_r, 0.0, f, scale),
                Some(inner),
                dim,
            ),
            inner: (inner, inner_r),
    ...
    BoxSlots {
        lower: match fill.map(|c| AtlasKey::hbar(h, radius, 0.0, c, scale)) {
            Some(k) => SlotPlan::snap(k, Some(rect), dim),
            None => SlotPlan::hidden(),
        },
        upper: match border.map(|c| AtlasKey::hbar(h, radius, t, c, scale)) {
            Some(k) => SlotPlan::snap(k, Some(rect), dim),
            None => SlotPlan::hidden(),
        },
        inner: (rect, radius),
...
pub(crate) fn box_plan(node: &Node, scale: f32) -> PartPlan {
    ...
    let slots = box_slots((0.0, 0.0, w, h), radius, bg, stroke, t, dim, scale);
    ...
    PartPlan::new([w, h, 0.0])
        .below(box_slot::LOWER, slots.lower)
        .below(box_slot::UPPER, slots.upper)
        .optional()
}
...
pub(crate) fn caption_plan(node: &Node, scale: f32) -> PartPlan {
    let (w, h) = (node.rect.w, node.rect.h);
    let wash = super::caption::hot_wash(node, Rect::from_xywh(0.0, 0.0, w, h)).map_or_else(SlotPlan::hidden, |(r, radius, c)| {
...
pub(crate) fn bar_plan(node: &Node, scale: f32) -> PartPlan {
    ...
    // The tint follows the CARD, not the box: cut to the inner rect when the
    // border is a ring, it washes the card and leaves the ring its own colour.
    // Cut to the outer rect it would wash the border too.
    let (tint_rect, tint_r) = slots.inner;
    let tint_c = theme::with_alpha(sev.color(), 0.10);
    let tint =
        AtlasKey::hbar(tint_rect.3, tint_r, 0.0, tint_c, scale).snap_at(Some(tint_rect), dim);
    ...
    let wash = super::info_bar::close_rect(w, h, x.bar_closable)
        .filter(|_| hot).map_or_else(SlotPlan::hidden, |c| {
            let a = if node.pressed { 0.10 } else { 0.06 };
            AtlasKey::hbar(c.height(), r, 0.0, theme::w(a), scale).snap_at(Some(r4(c)), dim)
            .fading()
        });

    // A ring puts the border UNDER the card it surrounds; a stroked border has
    // to stay over both the card and its tint.
    let (a, b, c) = if slots.ringed {
        (slots.lower, slots.upper, tint)
    } else {
        (slots.lower, tint, slots.upper)
    };
    PartPlan::new([w, h, 0.0])
        .below(0, a)
        .below(1, b)
...
pub(crate) fn select_plan(node: &Node, scale: f32) -> PartPlan {
    ...
    PartPlan::new([w, h, 0.0])
        .below(select_slot::LOWER, slots.lower)
        .below(select_slot::UPPER, slots.upper)
        .above(
            select_slot::INK,
            AtlasKey::hbar(h, ink_radius(node), 0.0, theme::w(1.0), scale)
                .snap_at(box_rect, ink_target(node))
...
pub(crate) fn editor_plan(node: &Node, scale: f32) -> PartPlan {
    ...
    let divider = match (node.kind, super::editor::spin_boxes(w, h)) {
        (ControlKind::NumberBox, Some((up, _))) => {
            ...
            let y = theme::SPACE_4;
            AtlasKey::solid(theme::over(theme::stroke(), theme::surface_raised()), scale)
                .snap_at(
    ...
    PartPlan::new([w, h, 0.0])
        .below(editor_slot::LOWER, slots.lower)
        .below(editor_slot::UPPER, slots.upper)
...
fn slider_sync(
    ...
    ensure(comp, node, slider_slot::N_BELOW, slider_slot::N_ABOVE);
...
fn slider_apply(parts: &mut Parts, g: &SliderGeom, snap: bool) {
    ...
    slider_fill_follow(parts, g.anchor);
    ...
    let batch = parts.above[2].open_batch();
...
fn slider_fill_static(parts: &mut Parts, r: (f32, f32, f32, f32)) {
    ...
    if parts.fill_live.replace(false) {
        fill.vis.stop_animation("Offset.X");
        fill.vis.stop_animation("Size.X");
...
/// Arming can no longer fail — every call it makes is infallible — so this
/// returns nothing and the caller has no fallback branch. The `static` path it
/// used to fall back to is still reached, by the two callers that want a plain
/// rect on purpose.
fn slider_fill_follow(parts: &mut Parts, anchor: (f32, f32, f32)) {
    let (_x0, _x1, origin_x) = anchor;
    if !parts.fill_live.get() {
        let comp = parts.above[0].vis.compositor();
        // The thumb flows IN as the expression's reference parameter — `Visual`
        // implements `Object`, so the sibling sprite is bound by name without a
        // raw `CompositionObject` ever surfacing.
        let thumb = &parts.above[2].vis;
        ...
        for (prop, text) in [
            ...
            let expr = comp.create_expression_animation(&text);
            expr.set_reference_parameter("t", thumb);
            parts.above[0].vis.start_animation(prop, &expr);
        }
    }
...
fn slider_fill_settle(parts: &mut Parts, batch: CompositionScopedBatch, r: (f32, f32, f32, f32)) {
    let generation = parts.fill_gen.get().wrapping_add(1);
    parts.fill_gen.set(generation);
    let settled = {
        // One owned `Visual` now carries both what the raw path needed an
        // `IVisual` and an `ICompositionObject` for.
        ...
        batch
            .on_completed(move || {
                if cell.get() == generation {
                    vis.stop_animation("Offset.X");
                    vis.stop_animation("Size.X");
                    vis.set_offset(r.0, r.1, 0.0);
                    vis.set_size(r.2, r.3);
                    live.set(false);
                }
            })
            .ok()
            .filter(|_| batch.try_end().is_ok())
    };
    match settled {
...
fn meter_arm_clip(parts: &mut Parts, w: f32) {
    ...
    let comp = parts.above[3].vis.compositor();
    let clip = match &parts.clip {
        Some(c) => c.clone(),
        None => {
            let c = comp.create_inset_clip();
            parts.above[0].vis.set_clip(Some(&c));
            c
        }
    };
    // Needle core is 2 DIPs wide at `nx - 1` → its centre is Offset.X + 1.
    let expr = comp.create_expression_animation(&format!(
        "Max(0.0, {w:.2} - (n.Offset.X + 1.0))"
    ));
    expr.set_reference_parameter("n", &parts.above[3].vis);
    // Started on the CLIP, not on a visual: a clip is not a visual, and
    // `RightInset` is the clip's own animatable property. This is the reveal —
    // the fill is held still and the clip swept across it.
    clip.start_animation("RightInset", &expr);
...
pub(crate) fn segmented_plan(node: &Node, scale: f32) -> PartPlan {
    ...
    // Both tray fills are washes (`w(0.06)` / `stroke_subtle`), so this always
    // takes the stroked-raster arm today; it goes through the same helper so a
    // future opaque tray is not a second place to get the rule right.
    let tray_slots = box_slots(
        (0.0, 0.0, w, h),
        tray_radius,
        Some(tray_bg),
        Some(theme::stroke()),
    ...
    PartPlan::new([w, h, edges_sig])
        .below(0, tray_slots.lower)
        .below(1, tray_slots.upper)
...
/// [`PROGRESS_CYCLE_SECS`] as a [`Duration`](std::time::Duration), for
/// [`super::ring_shape`] — the wrapper's key-frame `set_duration` takes one.
pub(crate) fn progress_cycle() -> std::time::Duration {
    secs(PROGRESS_CYCLE_SECS)
}

/// The bar's lane height, mirroring the retired `paint_progress_bar`.
fn progress_bar_h(node_h: f32) -> f32 {
    node_h.clamp(4.0, 6.0)
}
...
fn progress_sweep(node: &mut Node, relaid: bool) {
    ...
    if ind {
        ...
        if relaid || !parts.looping {
            ...
            parts.below[2].loop_x(-seg_w, w, PROGRESS_CYCLE_SECS);
            parts.looping = true;
        }
...
pub(crate) struct Caret {
    sprite: SpriteVisual,
    /// The sprite as a plain `Visual` — property writes and the blink's
    /// start/stop both go through it. One field where the raw path needed an
    /// `IVisual` and an `ICompositionObject`; see [`Part::vis`].
    vis: Visual,
    /// The atlas source currently bound + the epoch it came from.
    key: Option<AtlasKey>,
    epoch: u32,
...
impl Caret {
    ...
    /// its painted surface.
    fn new(comp: &Compositing, node: &Node) -> Self {
        let sprite = comp.new_sprite();
        let vis = (**sprite).clone();
        node.container.children().insert_at_top(&vis);
        Self { sprite, vis, key: None, epoch: 0, rect: None, shown: false }
    }
    ...
    fn start_blink(&mut self, comp: &Compositing) {
        self.vis.set_visible(true);
        self.shown = true;
        let interval = unsafe { crate::system_bindings::GetCaretBlinkTime() };
        if interval == 0 || interval == u32::MAX {
            // Blinking disabled: a solid caret.
            self.vis.stop_animation("Opacity");
            self.vis.set_opacity(1.0);
        } else {
            self.blink(comp, interval);
        }
    }
    ...
    fn blink(&self, comp: &Compositing, interval_ms: u32) {
        let a = comp.compositor().create_scalar_key_frame_animation();
        let cycle_s = interval_ms as f32 * 2.0 / 1000.0;
        a.set_duration(secs(cycle_s));
        a.set_iterate_forever();
        ...
        a.insert_key_frame(0.0, 1.0);
        a.insert_key_frame(0.5 - edge, 1.0);
        a.insert_key_frame(0.5, 0.0);
        a.insert_key_frame(1.0, 0.0);
        self.vis.start_animation("Opacity", &a);
    }
}
...
pub(crate) fn sync_caret(comp: &Compositing, atlas: &mut Atlas, node: &mut Node, scale: f32) {
    ...
    if node.caret.is_none() {
        node.caret = Some(Caret::new(comp, node));
    }
