...
use windows_composition::BatchKind;
...
impl DCompBackend {
    ...
    fn activate(&mut self, id: ControlId) {
        ...
        match kind {
            ControlKind::ToggleSwitch => {
                let on = !self.node(id).is_some_and(|n| n.ctrl().is_on);
                if let Some(n) = self.node_mut(id) {
                    n.ctrl_mut().is_on = on;
                    n.mark_dirty();
                }
            ...
            ControlKind::CheckBox | ControlKind::ToggleButton => {
                let on = !self.node(id).is_some_and(|n| n.ctrl().is_checked);
                if let Some(n) = self.node_mut(id) {
            ...
            ControlKind::Expander => {
                let ex = !self.node(id).is_some_and(|n| n.ctrl().expanded);
                if let Some(n) = self.node_mut(id) {
    ...
    fn host_flyout_content(&mut self, root: ControlId) {
        ...
        // `(*n.container).clone()`: the container derefs to the `Visual` the
        // popup adopts, and the deref is spelled out because the inherent
        // `Clone` on `ContainerVisual` wins method resolution otherwise.
        let Some(vis) = self.arena.get(root).map(|n| (*n.container).clone()) else {
            return;
        };
        popup.adopt(&vis);
    ...
    pub(crate) fn refresh_suggest(&mut self, owner: ControlId) {
        ...
        if mine {
            if let Some(p) = &mut self.popup {
                p.update_items(&self.comp, &mut self.glyphs, rows);
            }
            return;
        }
        let rect = self
    ...
    pub(crate) fn close_popup(&mut self) {
        if let Some(p) = self.popup.take() {
            ...
            if let Some(root) = self.hosted_flyout.take()
                && let Some(vis) = self.arena.get(root).map(|n| (*n.container).clone())
            {
            ...
            // The batch is opened BEFORE the fade is started, so the fade is
            // work the batch tracks — that ordering is the whole mechanism and
            // must not be reversed.
            let batch = self.comp.compositor().create_scoped_batch(BatchKind::Animation);
    ...
    pub(crate) fn on_wheel(&mut self, x: f32, y: f32, delta: i32) {
        ...
        if let Some(id) = self.scroll_at(x, y) {
            ...
            let scale = self.scale();
            let max = self.node(id).map_or(0.0, |n| (n.ctrl().content_h - n.rect.h).max(0.0));
            if let Some(n) = self.node_mut(id) {
    ...
    fn spin_hit(&mut self, id: ControlId, x: f32, y: f32) -> bool {
        // The same boxes the chevron sprites are placed into, so a press can
        // only ever land on the arrow it appears to land on.
        let Some(n) = self.node(id) else {
            return false;
        };
        let (origin, boxes) = (
            (n.rect.x, n.rect.y),
            editor::spin_boxes(n.rect.w, n.rect.h),
        );
        let Some((up, down)) = boxes else {
            return false;
        };
        ...
        // The two boxes tile the column, so the split between them is the only
        // question left.
        self.number_step(id, if ly < down.top { 1.0 } else { -1.0 }, false);
        true
    }
    ...
    fn focus_arrow(&mut self, dir: i32) {
        ...
        match self.node(id).map(|n| n.kind) {
            ...
            Some(ControlKind::SelectorBar) => {
                let (cur, n) = self
                    .node(id)
                    .map_or((0, 0), |nd| (nd.ctrl().selected_index, nd.ctrl().items.len() as i32));
                if n > 0 {
    ...
    pub(crate) fn uia_set_expanded(&mut self, id: ControlId, want: bool) {
        match self.node(id).map(|n| n.kind) {
            Some(ControlKind::Expander) => {
                let cur = self.node(id).is_some_and(|n| n.ctrl().expanded);
                if cur != want {
                    self.activate(id);
                }
            }
