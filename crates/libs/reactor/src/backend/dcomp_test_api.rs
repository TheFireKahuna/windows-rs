...
//! `test` is enabled, so it never exists in a shipping
//! build.
//!
...
impl ArenaHarness {
    pub fn new() -> windows_core::Result<Self> {
        ...
        Ok(Self {
            compositor: windows_composition::Compositor::new()?,
...
impl ArenaHarness {
    ...
    pub fn button_boxes(&self, id: ControlId) -> Option<ButtonBoxProbe> {
        ...
        let r = windows_canvas::Rect::from_xywh(0.0, 0.0, n.rect.w, n.rect.h);
        let b = dcomp::controls::button_boxes(n, r);
        let t = |r: windows_canvas::Rect| (r.left, r.top, r.right, r.bottom);
        Some(ButtonBoxProbe {
            icon: b.icon.map(t),
            badge: b.badge.map(t),
            label: t(b.label),
    ...
    pub fn part_plan(&self, id: ControlId, scale: f32) -> Option<PartPlanProbe> {
        ...
        let plan = match n.kind {
            ...
            ControlKind::ComboBox | ControlKind::DropDownButton => {
                dcomp::parts::select_plan(n, scale)
            }
    ...
    pub fn run_animation(&mut self, id: ControlId, cfg: &AnimationConfig) {
        ...
        dcomp::animate::start(&self.compositor, &node.container, cfg, None);
    }
    ...
    pub fn set_visual_state(&mut self, id: ControlId, opacity: f32, scale: f32) {
        let Some(node) = self.arena.get(id) else { return };
        node.container.set_opacity(opacity);
        node.container
            .set_scale(windows_numerics::Vector3::new(scale, scale, 1.0));
    }

    /// The node visual's current Opacity, straight off the compositor object.
    pub fn opacity(&self, id: ControlId) -> Option<f32> {
        Some(self.arena.get(id)?.container.opacity())
    }
    ...
    pub fn scale(&self, id: ControlId) -> Option<f32> {
        Some(self.arena.get(id)?.container.scale().x)
    }
