//! The reactor color type.
//!
//! [`Color`] is **linear scRGB, extended range** — `{ r, g, b, a: f32 }` where the
//! RGB channels are in linear light (Rec. 709 primaries) and may fall outside
//! `[0, 1]` (negative for wide-gamut, `> 1` for HDR headroom). It is the single
//! currency every component sets a color in; the backends adapt it to their target:
//!
//! - the DirectComposition backend draws to a **linear FP16 scRGB** surface and
//!   consumes the value raw (a passthrough `Color → ColorF`);
//! - the WinUI backend hands colors to `Microsoft.UI.Xaml` as 8-bit sRGB, so it
//!   linear→sRGB **encodes** (+ clamps) at that boundary ([`Color::to_srgb8`]).
//!
//! Authoring helpers ([`Color::rgb`] / [`Color::rgba`]) take legacy 8-bit **sRGB**
//! channels and gamma-decode them, so a hex literal *means* that linear color; the
//! decode is a `const` table lookup so it stays usable in `const` token definitions.
//! [`Color::scrgb`] takes raw linear channels (the extended-range / headroom path).

/// A color in **linear scRGB**, extended range (channels may be `< 0` or `> 1`).
///
/// Construct from legacy sRGB via [`rgb`](Self::rgb) / [`rgba`](Self::rgba) (decoded
/// to linear) or from raw linear channels via [`scrgb`](Self::scrgb) / [`new`](Self::new).
#[derive(Copy, Clone, Debug, Default, PartialEq)]
pub struct Color {
    /// Red, linear light (extended range).
    pub r: f32,
    /// Green, linear light (extended range).
    pub g: f32,
    /// Blue, linear light (extended range).
    pub b: f32,
    /// Alpha (opacity), linear `0.0..=1.0`.
    pub a: f32,
}

impl Color {
    /// A raw **linear** scRGB color (extended range). The HDR / wide-gamut path —
    /// pass linear channels directly (e.g. `1.0` = SDR paper-white, `> 1.0` = specular
    /// headroom).
    pub const fn scrgb(r: f32, g: f32, b: f32, a: f32) -> Self {
        Self { r, g, b, a }
    }

    /// A raw **linear** scRGB color. Alias of [`scrgb`](Self::scrgb); the channels are
    /// linear light, not sRGB.
    ...
    /// A color from 8-bit **sRGB** channels plus 8-bit alpha — the RGB channels are
    /// gamma-decoded to linear; alpha is a straight `a/255` (already linear). `const`.
    ...
    /// Encode this linear color to 8-bit **sRGB** `(r, g, b, a)` for a WinUI /
    /// `B8G8R8A8` output surface: clamp the extended-range channels to the `[0, 1]`
    /// SDR gamut, apply the sRGB OETF to RGB, and scale to bytes; alpha is clamped and
    /// scaled but left linear. HDR headroom (`> 1.0`) clamps gracefully to white.
    pub fn to_srgb8(self) -> (u8, u8, u8, u8) {
        let enc = |c: f32| (linear_to_srgb(c.clamp(0.0, 1.0)) * 255.0).round() as u8;
        (
            enc(self.r),
            enc(self.g),
            enc(self.b),
...
/// `powf`, which is not `const`) so [`Color::rgb`] / [`Color::rgba`] can gamma-decode
/// in a `const` context — `const` chrome tokens are defined that way.
/// `SRGB8_TO_LINEAR[v]` equals `srgb_eotf(v as f32 / 255.0)`.
#[rustfmt::skip]
