...
//!
//! The resolved-config view is text a user must be able to read, select and copy but never
//! edit. It is not a field with editing turned off — TSF would still own it and a touch
//! keyboard would raise for a surface with nothing to type into — so it publishes this
//! instead, and creates no text-services object at all.
//!
//! The blob is UTF-16, which is what makes this small: automation's text offsets *are*
//! UTF-16 offsets, so a range is two integers into a slice the tree already holds. Storing
//! the strings as `str` would cost an offset table per element to say the same thing.
...
///
/// Its endpoints are atomics because automation mutates a range in place — `Move`,
/// `ExpandToEnclosingUnit` and `MoveEndpointByUnit` all edit the object they are called on
/// — and the object is shared and callable from any thread, exactly like a provider.
pub struct Range {
    shared: Weak<Shared>,
    owner: ControlId,
    start: AtomicU32,
...
impl Range {
    ...
    /// The element's whole text, and this range's endpoints clamped into it.
    ///
    /// Clamped on every read rather than on every write: the text can be replaced by a
    /// republish while a client still holds a range over the old one, and a stale endpoint
    /// must read as a short range rather than as a panic or a slice of the next element's
    /// string.
...
/// Where a unit boundary sits, moving `count` of them from `from`.
///
/// Word boundaries are whitespace transitions and line boundaries are newlines, which is
/// the whole of what a single-line read-only surface has. `Format` collapses onto
/// `Document` because the text carries one paint — a run *is* a maximal same-paint span,
/// so a format boundary and a document boundary are the same boundary here.
...
/// The next position across a boundary the predicate defines.
...
/// The endpoint a call names, as a `(start, end)` index.
...
/// The span of a range automation handed back, when it is one of **ours over the same
/// element**.
///
/// Both halves matter. A foreign implementation or a marshalling proxy does not answer the
/// dynamic-cast protocol, so it resolves to nothing rather than to a wild reinterpretation
/// of somebody else's object; and endpoint arithmetic against a range over a *different*
/// element is meaningless, so it is refused rather than silently answered.
...
impl crate::bindings::ITextRangeProvider_Impl for Range_Impl {
    ...
    fn FindAttribute(
        ...
        // has a different one. `S_OK` with nothing is the documented "not found".
        Err(none())
    }
    ...
    fn GetBoundingRectangles(&self) -> Result<*mut SAFEARRAY> {
        // The element's own box, which is the truthful answer for a single-run surface:
        // one line, one paint, one rectangle. A sub-line rectangle would need cluster
        // geometry from the text engine, which lives on the front thread.
        let shared = self.shared.upgrade().ok_or_else(gone)?;
        let tree = super::element::tree_of(&shared);
        let at = tree.index_of(self.owner).ok_or_else(gone)?;
    ...
    fn Move(&self, unit: TextUnit, count: i32) -> Result<i32> {
        ...
        // A moved range is degenerate, then expanded — which is the documented behaviour
        // and the reason a client can page through a document a word at a time.
        let (to, _) = walk(&text, from, unit, 1);
        self.set(from, to);
        Ok(moved)
    }
    ...
    fn Select(&self) -> Result<()> {
        // Selection is the user's, driven by the same drag the editor uses. A client
        // cannot set it here, and saying so is better than pretending it worked.
        Err(none())
    }
...
/// A `BSTR`'s units. Automation hands us one and we index it, so it is a slice and never a
/// `String`.
...
/// A case fold good enough for a search over user-visible labels: ASCII, plus the Latin-1
/// letters the palette of a settings surface actually contains. Full Unicode folding is a
/// table this crate has no other use for.
...
impl crate::bindings::ITextProvider_Impl for Element_Impl {
    fn GetSelection(&self) -> Result<*mut SAFEARRAY> {
        // No selection is reported: what is selected lives with the surface that
        // draws the highlight, and reporting an empty array is how a provider says
        // "nothing is selected" without claiming selection is unsupported.
        Ok(super::variant::provider_array(&[]))
    }
    ...
    fn RangeFromPoint(&self, _: &UiaPoint) -> Result<ITextRangeProvider> {
        // Point-to-offset needs cluster geometry, which is the text engine's and
        // lives on the front thread. A degenerate range at the start is the
        // documented fallback and is what a client uses to anchor a walk.
...
/// Which element a text call is about, and the state behind it.
///
/// Only a surface that declared itself readable publishes a document. The fragment root
/// does not implement the pattern at all — it is the window, it carries no text, and an
/// interface that can only ever refuse is one a client wastes a query on.
