...
//!
//! A retained sprite expresses a curve through these, with no drawing surface kept alive
//! for it. A composition **path** is immutable; the **geometry** holding one is not, so a
//! curve that reshapes re-points its geometry and keeps both the object and any trim
...
///
/// The path it holds can be replaced: a curve whose shape changes assigns a new path to
/// the same geometry, so the shape, the sprite and any running trim animation all survive
/// the change.
#[derive(Clone)]
...
/// that draws it.
///
/// An analytic rounded rectangle: D2D rasterizes it by the pixels it covers, so a border
/// ring or a pill drawn this way costs less than the same outline realized as a path mesh,
/// whose cost scales with the perimeter.
#[derive(Clone)]
...
impl CompositionSpriteShape {
    ...
    pub fn set_stroke_dashes(&self, dashes: &[f32]) {
        // The dash array is a live collection owned by the shape rather than a property to
        // assign, so re-dashing rewrites it in place and the shape keeps its own array.
    ...
    ///
    /// Without it, [`set_scale`](CompositionSpriteShape::set_scale) scales the stroke as
    /// well: a 1-DIP stroke at 1.5× draws 1.5 DIPs wide and stops being a hairline.
...
/// geometry wrapper does — so this type bridges the two and a caller passes the
/// `ID2D1Geometry` it already holds.
...
impl bindings::IGeometrySource2DInterop_Impl for GeometrySource_Impl {
    ...
    fn TryGetGeometryUsingFactory(
        ...
        // The contract is "return geometry belonging to the factory passed in", and a
        // geometry cannot be moved between factories. Returning the geometry held here is
        // correct exactly when it already belongs to that factory, which is the affinity
        // rule `create_path` requires of its caller.
        Ok(self.0.clone())
    }
}
...
impl Compositor {
    ...
    /// crate crosses this boundary.
    ///
    /// # Errors
    ///
    /// Returns an error when `geometry` is not a Direct2D geometry, so a caller learns it
    /// passed the wrong object here rather than inside the compositor.
    ///
    /// # Factory affinity
    ///
    /// `geometry` must come from the same Direct2D factory as the device given to
    ...
    /// choosing, and neither side of that callback can verify the match: a mismatch
