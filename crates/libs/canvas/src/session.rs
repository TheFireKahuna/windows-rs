...
impl<'a> DrawingSession<'a> {
    ...
    pub fn from_borrowed_context(context: &'a ID2D1DeviceContext, offset: Matrix3x2) -> Self {
        debug_assert!(
            ...
            "offset must be a pure translation: transform() decomposes it by negating m31/m32"
        );
    ...
    ///
    /// Text rendering params (see `text::text_rendering_params`) use a
    ...
    /// stamped from the session, so through a
    /// [`create_tiling_brush`](Self::create_tiling_brush) one texel lands on
