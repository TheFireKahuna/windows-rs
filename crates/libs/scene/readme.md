...
a buffer the app presents itself) and an `Xform`. Everything drawable is a composition of those, so
the crate holds a fixed set of kinds rather than a cross-product of kinds and properties.
...
and there is no way to apply a patch without saying which display it is for.
...
all, because publishing happens when a tick ends and nothing asks for a tick.
