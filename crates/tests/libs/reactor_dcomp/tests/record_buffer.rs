...
/// `create` is the one eagerly-applied call (so `get_native_element` stays
/// exact mid-reconcile); every other call defers into the buffer.
#[test]
fn create_is_eager_everything_else_defers() {
    let mut rec = Recorder::new();

    let id = rec.backend().create(ControlKind::Border);
    assert_eq!(rec.pending(), 0, "create must not buffer");
    assert_eq!(rec.applied(), vec![format!("create {} Border", id.get())]);

    rec.backend()
        .set_prop(id, Prop::Opacity, &PropValue::F64(0.5));
    assert_eq!(rec.pending(), 1, "set_prop must buffer");
    assert_eq!(
        rec.applied().len(),
        1,
        "set_prop must not reach the backend before flush"
    );

    rec.flush();
    assert_eq!(rec.pending(), 0);
    assert_eq!(rec.applied().len(), 2);
    assert!(rec.applied()[1].starts_with("set_prop"));
}
...
fn replay_preserves_order_and_does_not_coalesce() {
    ...
    assert_eq!(
        rec.pending(),
        3,
        "identical-prop writes must not collapse in the buffer"
    );
    ...
    assert_eq!(
        applied.len(),
        4,
        "all three writes must replay: {applied:?}"
    );
...
fn side_table_round_trips_an_event_handler() {
    ...
    assert_eq!(rec.pending(), 1, "attach_event must buffer");
