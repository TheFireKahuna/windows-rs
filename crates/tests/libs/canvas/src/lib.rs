...
mod tests {
    ...
    fn color_constructors() {
        ...
        // The 8-bit constructors take *sRGB* components and gamma-decode them, so a
        // `ColorF` is always linear. Mid-scale sRGB 128 is linear ~0.216, not 0.5;
        // the endpoints are fixed points of the transfer function.
        let c3 = ColorF::from_rgba8(255, 128, 0, 255);
        assert_eq!(c3.r, 1.0);
        assert!(c3.g > 0.21 && c3.g < 0.22, "sRGB 128 → linear, got {}", c3.g);
        assert_eq!(c3.b, 0.0);
        // Alpha is linear coverage, not a color channel — passed through as-is.
        assert_eq!(c3.a, 1.0);
    ...
    fn multi_threaded_shared_device_stress() {
        let device = GpuDevice::new_warp_multi_threaded()
            .unwrap()
            .shared()
            .expect("a multi-threaded factory device must promote to shared");
