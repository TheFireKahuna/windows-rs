...
fn the_uri_launch_seam_is_inert_until_the_app_installs_a_launcher() {
    ...
    assert!(launch_uri("https://example.com"), "an accepted URI reported unhandled");
    assert_eq!(seen(), vec!["https://example.com"], "the URI did not reach the launcher");
    ...
    for bad in [
        ...
        "https://example.com/\u{2028}console.log(1)",
        "https://example.com/\u{2029}next",
        // Separators to anything tokenising on Unicode whitespace, and all but
        // invisible in the string a reviewer read.
        "https://example.com/\u{a0}--flag",
        "https://example.com/\u{3000}arg",
        // Render-vs-parse divergence: what the user sees is not what launches.
        "https://example.com/\u{202e}kcatta",
        "https://example.com\u{200b}.evil.test/",
        "https://example.com/\u{ad}soft",
    ] {
        assert!(
            !launch_uri(bad),
    ...
    assert!(launch_uri("https://example.com"));
    assert_eq!(seen(), vec!["https://example.com"]);
}
